...
module Crawdad
  ...
  class HtmlTokenizer
    ...
    #
    def optional_hyphen
      hyphen = @font_profile.width_of('-') # TODO: Get width of hyphen
    ...
    def word_segment(word, hyphenator)
      ...
      if hyphenator
        ...
        [0, *splits].each_cons(2) do |a, b|
          seg = word[a...b]
          tokens << box(@font_profile.width_of(seg), seg) # TODO Width of
          tokens += optional_hyphen
        end

        last = word[(splits.last || 0)..-1]
        tokens << box(@font_profile.width_of(last), last)
      else
        tokens << box(@font_profile.width_of(word), word)
      end
